...
export const uint = (bits: number): Ty<number> => ({
...
export const int = (bits: number): Ty<number> => ({
    ...
    tsTypes: () => [`int(${bits})`],
    print: (n) => "int" + String(n),
});

export const delta = (n: number, ty: Ty<number>): Ty<number> => ({
...
export const slice = (bits: Ty<number>, pad: number): Ty<Buffer> => {
    const load = (s: Slice) => {
...
export const seq1 = <T>(ty: Ty<T>): Ty<[T]> => ({
...
export const seq2 = <T, U>(t1: Ty<T>, t2: Ty<U>): Ty<[T, U]> => {
    ...
    return {
        ...
        tsTypes: () => [...t1.tsTypes(), ...t2.tsTypes()],
    };
};

export const seq3 = <T, U, V>(t1: Ty<T>, t2: Ty<U>, t3: Ty<V>): Ty<[T, U, V]> => {
    ...
    return {
        ...
        tsTypes: () => [...t1.tsTypes(), ...t2.tsTypes(), ...t3.tsTypes()],
    };
};

// special case: [-5, 10]
export const tinyInt: Ty<number> = ({
    store: (t, b) => {
        if (t < -5 || t > 10) {
            throw new Error(`Number must be in range [-5, 10]: ${t}`);
        }
        uint4.store((t + 16) & 15, b);
    },
...
export const largeInt: Ty<bigint> = ({
    store: (t, b) => {
        const len = t === 0n ? 1 : t.toString(2).length + (t < 0n ? 0 : 1);
        const len2 = (((len + 7) / 8) | 0) - 2;
        if (len2 < 0 || len2 >= 32) {
            throw new Error(`Wrong bitlength: ${len2}`);
        }
...
export const plduzArg: Ty<number> = {
...
export const xchgArgsAux = seq2(stack(4), stack(4));
...
export const xchgArgs: Ty<[number, number]> = {
    store: (t, b) => {
        validateXchg(t);
...
const instructions = {
    ...
    ADDINT: cat('add_mul', mkfixedn(0xa6, 8, 8, seq1(int(8)), "seq1(int(8))", `exec_add_tinyint8(_1, _2, false)`)),
    MULINT: cat('add_mul', mkfixedn(0xa7, 8, 8, seq1(int(8)), "seq1(int(8))", `exec_mul_tinyint8(_1, _2, false)`)),
    QADDINT: cat('add_mul', mkfixedn(0xb7a6, 16, 8, seq1(int(8)), "seq1(int(8))", `exec_add_tinyint8(_1, _2, true)`)),
    QMULINT: cat('add_mul', mkfixedn(0xb7a7, 16, 8, seq1(int(8)), "seq1(int(8))", `exec_mul_tinyint8(_1, _2, true)`)),
    EQINT: cat('int_cmp', mkfixedn(0xc0, 8, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x878, false, 'EQ')`)),
    LESSINT: cat('int_cmp', mkfixedn(0xc1, 8, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x887, false, 'LESS')`)),
    GTINT: cat('int_cmp', mkfixedn(0xc2, 8, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x788, false, 'GT')`)),
    NEQINT: cat('int_cmp', mkfixedn(0xc3, 8, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x787, false, 'NEQ')`)),
    QEQINT: cat('int_cmp', mkfixedn(0xb7c0, 16, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x878, true, 'QEQ')`)),
    QLESSINT: cat('int_cmp', mkfixedn(0xb7c1, 16, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x887, true, 'QLESS')`)),
    QGTINT: cat('int_cmp', mkfixedn(0xb7c2, 16, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x788, true, 'QGT')`)),
    QNEQINT: cat('int_cmp', mkfixedn(0xb7c3, 16, 8, seq1(int(8)), "seq1(int(8))", `exec_cmp_int(_1, _2, 0x787, true, 'QNEQ')`)),
    PUSHPOW2DEC: cat('int_const', mkfixedn(0x84, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_push_pow2dec`)),
    PUSHNEGPOW2: cat('int_const', mkfixedn(0x85, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_push_negpow2`)),
    FITS: cat('shift_logic', mkfixedn(0xb4, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_fits_tinyint8(_1, _2, false)`)),
    UFITS: cat('shift_logic', mkfixedn(0xb5, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_ufits_tinyint8(_1, _2, false)`)),
    QFITS: cat('shift_logic', mkfixedn(0xb7b4, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_fits_tinyint8(_1, _2, true)`)),
    QUFITS: cat('shift_logic', mkfixedn(0xb7b5, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_ufits_tinyint8(_1, _2, true)`)),
    SETCONTCTRMANY: version(9, cat('continuation_change', mkfixedn(0xede3, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_setcont_ctr_many`))),
    CALLCCARGS: cat('continuation_jump', mkfixedn(0xdb36, 16, 8, seq2(uint(4), uint(4)), "seq2(uint(4), uint(4))", `exec_callcc_args`)),
    TRYARGS: cat('exception', mkfixedn(0xf3, 8, 8, seq2(uint(4), uint(4)), "seq2(uint(4), uint(4))", `exec_try`)),
    PLDREFIDX: cat('cell_deserialize', mkfixedn(0xd74c >> 2, 14, 2, seq1(uint(2)), "seq1(uint(2))", `exec_preload_ref_fixed`)),
    CHASHI: version(6, cat('cell_deserialize', mkfixedn(0xd768 >> 2, 14, 2, seq1(uint(2)), "seq1(uint(2))", `exec_cell_hash_i(_1, _2, false)`))),
    CDEPTHI: version(6, cat('cell_deserialize', mkfixedn(0xd76c >> 2, 14, 2, seq1(uint(2)), "seq1(uint(2))", `exec_cell_depth_i(_1, _2, false)`))),
    JMPDICT: cat('continuation_dict_jump', mkfixedn(0xf14 >> 2, 10, 14, seq1(uint(14)), "seq1(uint(14))", `exec_jmpdict`)),
    PREPAREDICT: cat('continuation_dict_jump', mkfixedn(0xf18 >> 2, 10, 14, seq1(uint(14)), "seq1(uint(14))", `exec_preparedict`)),
    THROWARG: cat('exception', mkfixedn(0xf2c8 >> 3, 13, 11, seq1(uint(11)), "seq1(uint(11))", `exec_throw_arg_fixed(_1, _2, 0x7ff, 0)`)),
    THROWARGIF: cat('exception', mkfixedn(0xf2d8 >> 3, 13, 11, seq1(uint(11)), "seq1(uint(11))", `exec_throw_arg_fixed(_1, _2, 0x7ff, 3)`)),
    THROWARGIFNOT: cat('exception', mkfixedn(0xf2e8 >> 3, 13, 11, seq1(uint(11)), "seq1(uint(11))", `exec_throw_arg_fixed(_1, _2, 0x7ff, 2)`)),

    JMPXARGS: cat('continuation_jump', mkfixedn(0xdb1, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_jmpx_args`)),
    RETARGS: cat('continuation_jump', mkfixedn(0xdb2, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_ret_args`)),
    RETURNARGS: cat('continuation_change', mkfixedn(0xed0, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_return_args`)),
    BLKDROP: cat('stack', mkfixedn(0x5f0, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_blkdrop`)),
    TUPLE: cat('tuple', mkfixedn(0x6f0, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_mktuple`)),
    INDEX: cat('tuple', mkfixedn(0x6f1, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_tuple_index`)),
    UNTUPLE: cat('tuple', mkfixedn(0x6f2, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_untuple`)),
    UNPACKFIRST: cat('tuple', mkfixedn(0x6f3, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_untuple_first`)),
    EXPLODE: cat('tuple', mkfixedn(0x6f4, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_explode_tuple`)),
    SETINDEX: cat('tuple', mkfixedn(0x6f5, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_tuple_set_index`)),
    INDEXQ: cat('tuple', mkfixedn(0x6f6, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_tuple_quiet_index`)),
    SETINDEXQ: cat('tuple', mkfixedn(0x6f7, 12, 4, seq1(uint(4)), "seq1(uint(4))", `exec_tuple_quiet_set_index`)),
    XC2PU: cat('stack', mkfixedn(0x541, 12, 12, seq3(stack(4), stack(4), stack(4)), "seq3(stack(4), stack(4), stack(4))", `exec_xc2pu`)),
    XCPU2: cat('stack', mkfixedn(0x543, 12, 12, seq3(stack(4), stack(4), stack(4)), "seq3(stack(4), stack(4), stack(4))", `exec_xcpu2`)),
    PUSH3: cat('stack', mkfixedn(0x547, 12, 12, seq3(stack(4), stack(4), stack(4)), "seq3(stack(4), stack(4), stack(4))", `exec_push3`)),
    XCHG2: cat('stack', mkfixedn(0x50, 8, 8, seq2(stack(4), stack(4)), "seq2(stack(4), stack(4))", `exec_xchg2`)),
    XCPU: cat('stack', mkfixedn(0x51, 8, 8, seq2(stack(4), stack(4)), "seq2(stack(4), stack(4))", `exec_xcpu`)),
    PUSH2: cat('stack', mkfixedn(0x53, 8, 8, seq2(stack(4), stack(4)), "seq2(stack(4), stack(4))", `exec_push2`)),
    PUXC: cat('stack', mkfixedn(0x52, 8, 8, seq2(stack(4), delta(-1, stack(4))), "seq2(stack(4), delta(-1, stack(4)))", `exec_puxc`)),
    XCPUXC: cat('stack', mkfixedn(0x542, 12, 12, seq3(stack(4), stack(4), delta(-1, stack(4))), "seq3(stack(4), stack(4), delta(-1, stack(4)))", `exec_xcpuxc`)),
    PUXC2: cat('stack', mkfixedn(0x544, 12, 12, seq3(stack(4), delta(-1, stack(4)), delta(-1, stack(4))), "seq3(stack(4), delta(-1, stack(4)), delta(-1, stack(4)))", `exec_puxc2`)),
    PUXCPU: cat('stack', mkfixedn(0x545, 12, 12, seq3(stack(4), delta(-1, stack(4)), delta(-1, stack(4))), "seq3(stack(4), delta(-1, stack(4)), delta(-1, stack(4)))", `exec_puxcpu`)),
    PU2XC: cat('stack', mkfixedn(0x546, 12, 12, seq3(stack(4), delta(-1, stack(4)), delta(-2, stack(4))), "seq3(stack(4), delta(-1, stack(4)), delta(-2, stack(4)))", `exec_pu2xc`)),
    BLKSWAP: cat('stack', mkfixedn(0x55, 8, 8, seq2(delta(1, uint(4)), delta(1, uint(4))), "seq2(delta(1, uint(4)), delta(1, uint(4)))", `exec_blkswap`)),
    REVERSE: cat('stack', mkfixedn(0x5e, 8, 8, seq2(delta(2, uint(4)), uint(4)), "seq2(delta(2, uint(4)), uint(4))", `exec_reverse`)),
    SETCONTARGS: cat('continuation_change', mkfixedn(0xec, 8, 8, seq2(uint(4), delta(-1, uint(4))), "seq2(uint(4), delta(-1, uint(4)))", `exec_setcontargs`)),
    BLESSARGS: cat('continuation_change', mkfixedn(0xee, 8, 8, seq2(uint(4), delta(-1, uint(4))), "seq2(uint(4), delta(-1, uint(4)))", `exec_bless_args`)),
    STIR: cat('cell_serialize', mkfixedn(0xcf0a, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    STUR: cat('cell_serialize', mkfixedn(0xcf0b, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    STIQ: cat('cell_serialize', mkfixedn(0xcf0c, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    STUQ: cat('cell_serialize', mkfixedn(0xcf0d, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    STIRQ: cat('cell_serialize', mkfixedn(0xcf0e, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    STURQ: cat('cell_serialize', mkfixedn(0xcf0f, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    PLDI: cat('cell_deserialize', mkfixedn(0xd70a, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    PLDU: cat('cell_deserialize', mkfixedn(0xd70b, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    LDIQ: cat('cell_deserialize', mkfixedn(0xd70c, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    LDUQ: cat('cell_deserialize', mkfixedn(0xd70d, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    PLDIQ: cat('cell_deserialize', mkfixedn(0xd70e, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    PLDUQ: cat('cell_deserialize', mkfixedn(0xd70f, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),

    PLDUZ: cat('cell_deserialize', mkfixedn(0xd710 >> 3, 13, 3, seq1(plduzArg), "seq1(plduzArg)", `exec_preload_uint_fixed_0e`)),
    PLDSLICE: cat('cell_deserialize', mkfixedn(0xd71d, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_slice_fixed2`)),
    LDSLICEQ: cat('cell_deserialize', mkfixedn(0xd71e, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_slice_fixed2`)),
    PLDSLICEQ: cat('cell_deserialize', mkfixedn(0xd71f, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_slice_fixed2`)),
    IFBITJMP: cat('continuation_cond_loop', mkfixedn(0xe380 >> 5, 11, 5, seq1(uint(5)), "seq1(uint(5))", `exec_if_bit_jmp`)),
    IFNBITJMP: cat('continuation_cond_loop', mkfixedn(0xe3a0 >> 5, 11, 5, seq1(uint(5)), "seq1(uint(5))", `exec_if_bit_jmp`)),
    INDEX2: cat('tuple', mkfixedn(0x6fb, 12, 4, seq2(uint(2), uint(2)), "seq2(uint(2), uint(2))", `exec_tuple_index2`)),
    INDEX3: cat('tuple', mkfixedn(0x6fc >> 2, 10, 6, seq3(uint(2), uint(2), uint(2)), "seq3(uint(2), uint(2), uint(2))", `exec_tuple_index3`)),

    PUSHPOW2: cat('int_const', mkfixedrangen(0x8300, 0x83ff, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_push_pow2`)),
    BLKPUSH: cat('stack', mkfixedrangen(0x5f10, 0x6000, 16, 8, seq2(uint(4), uint(4)), "seq2(uint(4), uint(4))", `exec_blkpush`)),
    BLKDROP2: cat('stack', mkfixedrangen(0x6c10, 0x6d00, 16, 8, seq2(uint(4), uint(4)), "seq2(uint(4), uint(4))", `exec_blkdrop2`)),
    GETGLOB: cat('config', mkfixedrangen(0xf841, 0xf860, 16, 5, seq1(uint(5)), "seq1(uint(5))", `exec_get_global`)),
    ...
    SETCONTCTR: cat('continuation_change', mkfixedrangen(0xed60, 0xed68, 16, 4, seq1(control), "seq1(control)", `exec_setcont_ctr`)),
    SETRETCTR: cat('continuation_change', mkfixedrangen(0xed70, 0xed78, 16, 4, seq1(control), "seq1(control)", `exec_setret_ctr`)),
    SETALTCTR: cat('continuation_change', mkfixedrangen(0xed80, 0xed88, 16, 4, seq1(control), "seq1(control)", `exec_setalt_ctr`)),
    POPSAVE: cat('continuation_change', mkfixedrangen(0xed90, 0xed98, 16, 4, seq1(control), "seq1(control)", `exec_popsave_ctr`)),
    SAVECTR: cat('continuation_change', mkfixedrangen(0xeda0, 0xeda8, 16, 4, seq1(control), "seq1(control)", `exec_save_ctr`)),
    SAVEALTCTR: cat('continuation_change', mkfixedrangen(0xedb0, 0xedb8, 16, 4, seq1(control), "seq1(control)", `exec_savealt_ctr`)),
    SAVEBOTHCTR: cat('continuation_change', mkfixedrangen(0xedc0, 0xedc8, 16, 4, seq1(control), "seq1(control)", `exec_saveboth_ctr`)),

    RUNVM: version(4, cat('continuation_jump', mkfixedn(0xdb4, 12, 12, seq1(runvmArg), "seq1(runvmArg)", `exec_runvm`))),
    ...
    "ADDRSHIFT#MOD": version(4, cat('div', mkfixedn(0xa930, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`))),
    "ADDRSHIFTR#MOD": version(4, cat('div', mkfixedn(0xa931, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`))),
    "ADDRSHIFTC#MOD": version(4, cat('div', mkfixedn(0xa932, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`))),
    "RSHIFT#": cat('div', mkfixedn(0xa934, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "RSHIFTR#": cat('div', mkfixedn(0xa935, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "RSHIFTC#": cat('div', mkfixedn(0xa936, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "MODPOW2#": cat('div', mkfixedn(0xa938, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "MODPOW2R#": cat('div', mkfixedn(0xa939, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "MODPOW2C#": cat('div', mkfixedn(0xa93a, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "RSHIFT#MOD": cat('div', mkfixedn(0xa93c, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "RSHIFTR#MOD": cat('div', mkfixedn(0xa93d, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "RSHIFTC#MOD": cat('div', mkfixedn(0xa93e, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shrmod(_1, _2, 2)`)),
    "MULADDRSHIFT#MOD": version(4, cat('div', mkfixedn(0xa9b0, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`))),
    "MULADDRSHIFTR#MOD": version(4, cat('div', mkfixedn(0xa9b1, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`))),
    "MULADDRSHIFTC#MOD": version(4, cat('div', mkfixedn(0xa9b2, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`))),
    "MULRSHIFT#": cat('div', mkfixedn(0xa9b4, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULRSHIFTR#": cat('div', mkfixedn(0xa9b5, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULRSHIFTC#": cat('div', mkfixedn(0xa9b6, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULMODPOW2#": cat('div', mkfixedn(0xa9b8, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULMODPOW2R#": cat('div', mkfixedn(0xa9b9, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULMODPOW2C#": cat('div', mkfixedn(0xa9ba, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULRSHIFT#MOD": cat('div', mkfixedn(0xa9bc, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULRSHIFTR#MOD": cat('div', mkfixedn(0xa9bd, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "MULRSHIFTC#MOD": cat('div', mkfixedn(0xa9be, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_mulshrmod(_1, _2, 2)`)),
    "LSHIFT#ADDDIVMOD": version(4, cat('div', mkfixedn(0xa9d0, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`))),
    "LSHIFT#ADDDIVMODR": version(4, cat('div', mkfixedn(0xa9d1, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`))),
    "LSHIFT#ADDDIVMODC": version(4, cat('div', mkfixedn(0xa9d2, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`))),
    "LSHIFT#DIV": cat('div', mkfixedn(0xa9d4, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#DIVR": cat('div', mkfixedn(0xa9d5, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#DIVC": cat('div', mkfixedn(0xa9d6, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#MOD": cat('div', mkfixedn(0xa9d8, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#MODR": cat('div', mkfixedn(0xa9d9, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#MODC": cat('div', mkfixedn(0xa9da, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#DIVMOD": cat('div', mkfixedn(0xa9dc, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#DIVMODR": cat('div', mkfixedn(0xa9dd, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),
    "LSHIFT#DIVMODC": cat('div', mkfixedn(0xa9de, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_shldivmod(_1, _2, 2)`)),

    // special case: named argument
    HASHEXT: version(4, cat('crypto', mkfixedn(0xf904, 16, 8, seq1(hash), "seq1(hash)", `exec_hash_ext`))),
    HASHEXTR: version(4, cat('crypto', mkfixedn(0xf905, 16, 8, seq1(hash), "seq1(hash)", `exec_hash_ext`))),
    HASHEXTA: version(4, cat('crypto', mkfixedn(0xf906, 16, 8, seq1(hash), "seq1(hash)", `exec_hash_ext`))),
    HASHEXTAR: version(4, cat('crypto', mkfixedn(0xf907, 16, 8, seq1(hash), "seq1(hash)", `exec_hash_ext`))),
    ...
    XCHG3: cat('stack', mkfixedn(0x4, 4, 12, seq3(stack(4), stack(4), stack(4)), "seq3(stack(4), stack(4), stack(4))", `exec_xchg3`)),
    // XCHG3: cat('stack', mkfixedn(0x540, 12, 12, seq3(stack(4), stack(4), stack(4)), "seq3(stack(4), stack(4), stack(4))", `exec_xchg3`)),
    STI: cat('cell_serialize', mkfixedn(0xca, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int(_1, _2, true)`)),
    // STI: cat('cell_serialize', mkfixedn(0xcf08, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    STU: cat('cell_serialize', mkfixedn(0xcb, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int(_1, _2, false)`)),
    // STU: cat('cell_serialize', mkfixedn(0xcf09, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_store_int_fixed`)),
    LDI: cat('cell_deserialize', mkfixedn(0xd2, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed(_1, _2, 0)`)),
    // LDI: cat('cell_deserialize', mkfixedn(0xd708, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    LDU: cat('cell_deserialize', mkfixedn(0xd3, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed(_1, _2, 1)`)),
    // LDU: cat('cell_deserialize', mkfixedn(0xd709, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_int_fixed2`)),
    LDSLICE: cat('cell_deserialize', mkfixedn(0xd6, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_slice_fixed`)),
    // LDSLICE: cat('cell_deserialize', mkfixedn(0xd71c, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_load_slice_fixed2`)),

    // special case: with or without arg
    LSHIFT: cat('shift_logic', mksimple(0xac, 8, `(_1) => exec_lshift(_1, false)`)),
    LSHIFT_1: cat('shift_logic', mkfixedn(0xaa, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_lshift_tinyint8(_1, _2, false)`)),
    QLSHIFT: cat('shift_logic', mksimple(0xb7ac, 16, `(_1) => exec_lshift(_1, true)`)),
    QLSHIFT_1: cat('shift_logic', mkfixedn(0xb7aa, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_lshift_tinyint8(_1, _2, true)`)),
    BCHKBITS: cat('cell_serialize', mksimple(0xcf39, 16, `(_1) => exec_builder_chk_bits_refs(_1, 1)`)),
    BCHKBITS_1: cat('cell_serialize', mkfixedn(0xcf38, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_builder_chk_bits(_1, _2, false)`)),
    BCHKBITSQ: cat('cell_serialize', mksimple(0xcf3d, 16, `(_1) => exec_builder_chk_bits_refs(_1, 5)`)),
    BCHKBITSQ_1: cat('cell_serialize', mkfixedn(0xcf3c, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_builder_chk_bits(_1, _2, true)`)),
    ...
    RSHIFT_1: cat('shift_logic', mkfixedn(0xab, 8, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_rshift_tinyint8(_1, _2, false)`)),
    // RSHIFT: cat('div', mksimple(0xa924, 16, `exec_shrmod(_1, _2, 0)`)),
    QRSHIFT: cat('shift_logic', mksimple(0xb7ad, 16, `(_1) => exec_rshift(_1, true)`)),
    QRSHIFT_1: cat('shift_logic', mkfixedn(0xb7ab, 16, 8, seq1(delta(1, uint(8))), "seq1(delta(1, uint(8)))", `exec_rshift_tinyint8(_1, _2, true)`)),
    ...
    DEBUG: cat('debug', mkfixedrangen(0xfe01, 0xfe14, 16, 8, seq1(uint(8)), "seq1(uint(8))", `exec_dummy_debug`)),
    ...
    // PUSH SHORT s*
    // PUSH LONG s*
    PUSHCTR: cat('continuation_change', mkfixedrangen(0xed40, 0xed48, 16, 4, seq1(control), "seq1(control)", `exec_push_ctr`)),
    PUSH: cat('stack', mkfixedrangen(0x22, 0x30, 8, 4, seq1(stack(4)), "seq1(stack(4))", `exec_push`)),
    PUSH_LONG: cat('stack', mkfixedn(0x56, 8, 8, seq1(stack(8)), "seq1(stack(8))", `exec_push_l`)),

    POPCTR: cat('continuation_change', mkfixedrangen(0xed50, 0xed58, 16, 4, seq1(control), "seq1(control)", `exec_pop_ctr`)),
    POP: cat('stack', mkfixedrangen(0x32, 0x40, 8, 4, seq1(stack(4)), "seq1(stack(4))", `exec_pop`)),
    POP_LONG: cat('stack', mkfixedn(0x57, 8, 8, seq1(stack(8)), "seq1(stack(8))", `exec_pop_l`)),

    CALLXARGS: cat('continuation_jump', mkfixedn(0xdb0, 12, 4, seq2(uint(4), minusOne), "seq2(uint(4), minusOne)", `exec_callx_args_p`)),
    CALLXARGS_1: cat('continuation_jump', mkfixedn(0xda, 8, 8, seq2(uint(4), uint(4)), "seq2(uint(4), uint(4))", `exec_callx_args`)),
