import {
    ...
    ABIReceiver,
    TupleBuilder,
    DictionaryValue, BitString
} from '@ton/core';
import {ADD, compile, DICTIGETJMPZ, PUSHDICTCONST, DUMPSTK, PUSHINT4, SETCP0, ADD_INT, BLKDROP2} from "./instr";
...
export function storeFoo$Data(src: Foo$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

export function loadFoo$Data(slice: Slice) {
    const sc_0 = slice;
    return { $$type: 'Foo$Data' as const };
}

function loadTupleFoo$Data(source: TupleReader) {
    return { $$type: 'Foo$Data' as const };
}

function loadGetterTupleFoo$Data(source: TupleReader) {
    return { $$type: 'Foo$Data' as const };
}

function storeTupleFoo$Data(source: Foo$Data) {
    const builder = new TupleBuilder();
    return builder.build();
}

function dictValueParserFoo$Data(): DictionaryValue<Foo$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeFoo$Data(src)).endCell());
        },
        parse: (src) => {
            return loadFoo$Data(src.loadRef().beginParse());
        }
    }
}
...
function initFoo_init_args(src: Foo_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}
...
async function Foo_init() {
    ...
    initFoo_init_args({ $$type: 'Foo_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

export const Foo_errors = {
...
export const Foo_errors_backward = {
    "Stack underflow": 2,
    "Stack overflow": 3,
    "Integer overflow": 4,
...
const Foo_types: ABIType[] = [
    ...
    {"name":"Foo$Data","header":null,"fields":[]},
]

const Foo_opcodes = {
}

const Foo_getters: ABIGetter[] = [
    {"name":"data","methodId":100194,"arguments":[],"returnType":{"kind":"simple","type":"int","optional":false,"format":257}},
]

export const Foo_getterMapping: { [key: string]: string } = {
    'data': 'getData',
}

const Foo_receivers: ABIReceiver[] = [
]
...
export class Foo implements Contract {
    ...
    public static readonly errors = Foo_errors_backward;
    public static readonly opcodes = Foo_opcodes;
    
    static async init() {
        return await Foo_init();
    }
    
    static async fromInit() {
        const __gen_init = await Foo_init();
        const address = contractAddress(0, __gen_init);
        return new Foo(address, __gen_init);
    }
